...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.cleaning_window = DatasetCleaning_Window()
    ...
    def go_cleaning(self):
        ...
        if getattr(self.loading, "current_df", None) is not None:
            ...
            self.cleaning_window.processed_path = processed_path
        else:
    ...
    def go_plotting(self):
        ...
        if getattr(self.cleaning_window, "merged_df", None) is not None:
            sheets_dfs = getattr(self.cleaning_window, "sheets_dfs", {})
            processed_path = getattr(self.cleaning_window, "processed_path", None)

            print(f"Passing data to plotting: shape={self.cleaning_window.merged_df.shape}")
            print(f"Number of sheets: {len(sheets_dfs)}")

            self.plotting.set_dataframes(self.cleaning_window.merged_df, sheets_dfs, processed_path)
        else:
