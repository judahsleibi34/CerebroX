...
#     QFrame, QComboBox, QScrollArea, QStyledItemDelegate
# )
...
#                         "Line Chart",
#                         numeric_cols[0],
#                         numeric_cols[1:][:3]  # First col as x, next 3 as y
#                     )
...
#                 }}
#             """)
#             cb.stateChanged.connect(lambda state, pt=plot_type: self._on_plot_toggled(pt, state))
...
#         summary_title.setStyleSheet(f"color: {PALETTE['text']}; font-size: 14px; font-weight: 600; margin-top: 8px;")
#         layout.addWidget(summary_title)
        
#         self.summary_label = QLabel("No plots generated yet. Select plot types above to begin.")
...
#                 self.output_dir = Path(self.processed_path).parent
...
#         """Set the dataframes from the cleaning window"""
...
#         # Reset sheet combo to merged only
#         self.sheet_combo.clear()
#         self.sheet_combo.addItem("All sheets (merged)")
...
#         print(f"Current dataframe shape: {self.current_df.shape if self.current_df is not None else 'None'}")
...
#         """Activate the plotting class/method for the given plot type"""
#         if self.current_df is None or self.current_df.empty:
...
#         # Update summary to show generation in progress
...
#         """Deactivate the plotting class/method for the given plot type"""
#         sheet_name = self.current_sheet if self.current_sheet else "All sheets (merged)"
...
#         self.summary_label.setText(f'<span style="color: #ff4444;">{error_msg}</span>')

#     def _update_summary(self):
#         """Update the summary label with plot generation status"""
#         if not self.completed_plots and not self.failed_plots:
#             self.summary_label.setText("No plots generated yet. Select plot types above to begin.")
#             return
        
#         self.summary_label.setText(self._build_summary())

#     def _build_summary(self):
#         """Build the summary HTML string"""
#         summary_parts = []
        
#         if self.completed_plots:
#             summary_parts.append(f'<span style="color: #00ff88;"><b>✓ Successfully Generated ({len(self.completed_plots)}):</b></span><br>')
#             summary_parts.append(", ".join(self.completed_plots))
        
#         if self.failed_plots:
#             if summary_parts:
#                 summary_parts.append("<br><br>")
#             summary_parts.append(f'<span style="color: #ff4444;"><b>✗ Failed ({len(self.failed_plots)}):</b></span><br>')
#             summary_parts.append(", ".join(self.failed_plots))
        
#         if self.output_dir and self.completed_plots:
#             summary_parts.append(f"<br><br>📁 <b>Output Folder:</b><br><span style='font-size: 11px;'>{self.output_dir}</span>")
        
#         return "".join(summary_parts)

#     def get_selected_plots(self):
#         """Get list of currently selected plot types"""
#         return [plot_type for plot_type, cb in self.plot_checkboxes.items() if cb.isChecked()]
...
class Plotting_Page(QWidget):
    ...
    def go_next(self):
        """Navigate to next step after plotting (hook for main window)."""
        main_window = self.window()
        # You can implement this in your main window, e.g. go_modeling()
        if hasattr(main_window, "go_next_from_plotting"):
            main_window.go_next_from_plotting()
    ...
    def _initialize_visualizer(self):
        ...
        try:
            if self.output_dir is None:
                # Prefer folder of processed CSV if available
                if self.processed_path:
                    self.output_dir = Path(self.processed_path).parent
                else:
                    # Fallback: default 'eda_plots' folder in current working directory
                    self.output_dir = Path.cwd() / "eda_plots"
                self.output_dir.mkdir(parents=True, exist_ok=True)

            self.visualizer = DataVisualizer(
                dataset_path="",          # not needed if df is passed
                output_dir=self.output_dir,
                df=self.current_df
            )
